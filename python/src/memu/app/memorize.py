...
class MemorizeMixin:
    ...
    async def _generate_entries_from_text(
        ...
        *,
        ...
        if not memory_types:
            return []
        client = llm_client or self._get_llm_client()
        prompt_pairs: list[tuple[MemoryType, str]] = []
        for mtype in memory_types:
            ...
            if prompt_text.strip():
                prompt_pairs.append((mtype, prompt_text))

        if not prompt_pairs:
            return []

        # These prompts are instructions that request structured output, not text summaries.
        tasks = [client.chat(prompt_text) for (_mtype, prompt_text) in prompt_pairs]
        responses = await asyncio.gather(*tasks)
        return self._parse_structured_entries([m for (m, _p) in prompt_pairs], responses)
    ...
    async def _preprocess_conversation(
        ...
        # Return one resource per segment.
        # IMPORTANT: avoid per-segment LLM summarization here — it can be slow and cause ingest timeouts.
        ...
        for segment in segments:
            ...
            # If the preprocessor returned a caption, keep it; otherwise omit.
            caption = segment.get("caption") if isinstance(segment, dict) else None
